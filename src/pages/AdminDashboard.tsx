...
import { Package, Home, TrendingUp, LogOut, Plus, Users, DollarSign, Calendar, CheckCircle } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const fetchDonations = async () => {
    try {
      ...
      const uniqueDonors = new Set(data.map((d: any) => d.donorId));
      const monetaryTotal = data
        .filter((d: any) => d.type === "monetary" && d.amount)
        .reduce((sum: number, d: any) => sum + Number(d.amount), 0);

      const now = new Date();
      const lastWeek = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
      const recentDists = distributions.filter((d: any) => new Date(d.createdAt) > lastWeek).length;
  ...
  const fetchHomes = async () => {
    try {
      const data = await apiClient.get("/children-homes");
      setHomes(data);
    } catch (error) {
  ...
  const updateDonationStatus = async (donationId: string, status: string) => {
    try {
      await apiClient.put(`/donations/${donationId}`, { status });
  ...
  const createDistribution = async () => {
    ...
    try {
      await apiClient.post("/distributions", {
        ...
        childrenHomeId: selectedHome,
        notes: distributionNotes,
      });
      await updateDonationStatus(selectedDonation._id, "supplied");
  ...
  const createHome = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post("/children-homes", {
        name: newHome.name,
        location: newHome.location,
        contactInfo: newHome.contact_info,
      });
      ...
      <div className="border-b bg-card/50 backdrop-blur supports-[backdrop-filter]:bg-card/50">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Manage donations and distributions</p>
          </div>
